...
$('foodForm').addEventListener('submit',async e=>{e.preventDefault();const name=$('productName').value.trim();const ingredients=$('ingredientsInput').value.trim();if(!name){showMessage('اسم المنتج مطلوب حتى يظهر بوضوح في نتيجة الفحص.');$('productName').focus();return}if(!ingredients&&!uploadedFile){showMessage('اكتبي المكونات أو ارفعي صورة المنتج لإجراء فحص مفيد.');return}const mode=uploadedFile?'image':'text';if(usage[mode]<=0){showMessage('انتهى رصيد الفحص التجريبي لهذا اليوم. يعود الرصيد غداً.');return}usage[mode]--;saveUsage();$('checkButton').disabled=true;let reading=null;if(uploadedFile)reading=await readImage();else $('result').innerHTML='<p class="loading">جاري فحص المنتج والمكونات...</p>';const imageData=reading?`${reading.ocrText} ${reading.labels.join(' ')}`:'';const matches=analyze(`${name} ${ingredients} ${imageData}`);renderResult(name,matches,reading);$('checkButton').disabled=false;if(uploadedFile)clearImage()});
